...
from active_patcher.base import LABEL_IGNORED
from active_patcher.exceptions import LearnerNotInitializedException
from active_patcher.utils.data import list_length
from active_patcher.utils.labels import concatenate, get_ignored_labels_mask, remove_by_index
from active_patcher.version import __version__ as version
...
class PoolBasedActiveLearner(AbstractPoolBasedActiveLearner):
    ...
    Parameters
    ----------
    clf_factory : active_patcher.classifiers.factories.AbstractClassifierFactory
        A factory responsible for creating new classifier instances.
    query_strategy : active_patcher.query_strategies.QueryStrategy
        Query strategy which is responsible for selecting instances during a `query()` call.
    dataset : ~active_patcher.data.datasets.Dataset
    ...
    def update(self, y, indices_validation=None):
        ...
        y : numpy.ndarray or scipy.sparse.csr_matrix
            ...
            row of labels to ` active_patcher.base import LABEL_IGNORED` will ignore the respective
            sample.
        indices_validation : numpy.ndarray, default=None
            The given indices (relative to `self.x_indices_labeled`) define a custom validation set
            if provided. Otherwise, each classifier that uses a validation set will be responsible
